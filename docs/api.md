...
params.trt.runtime_data     = ".../sam3_runtime.sam3rt"; // full .ggml is not opened
...
When `runtime_data` is set, `sam3_load_model` opens only the roughly 8 MB
`.sam3rt` sidecar generated by `scripts/development/export_onnx.sh`; it does
not open or scan `model_path`. The sidecar holds the tokenizer,
hyperparameters, and prompt/geometry helper tensors still used on the CPU.
